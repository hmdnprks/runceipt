...
describe("ElevationProfile", () => {
  it("renders an SVG with area and line paths", () => {
    const { container } = render(<ElevationProfile data={[5, 10, -3, 8]} theme={theme} />);
  ...
  it("line path starts with M and contains L", () => {
    const { container } = render(<ElevationProfile data={[5, 10, -3]} theme={theme} />);
    const paths = container.querySelectorAll("path");
  ...
  it("area path ends with Z (closed)", () => {
    const { container } = render(<ElevationProfile data={[5, -2]} theme={theme} />);
    const paths = container.querySelectorAll("path");
  ...
  it("handles single data point without crashing", () => {
    const { container } = render(<ElevationProfile data={[5]} theme={theme} />);
